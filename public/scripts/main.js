$(function(){
  ...
  function searchFunction() {
      ...
      $('#category_capitalCollections_popup').html("");
      ...
      //  });
       ...
       console.log('test');
         $('#capital_collections_results').addClass("category_capitalCollections_popup_open");
         $('#capital_collections_results').html("Results");
         ...
         // parse incoming data as XML.
          ...
          var numberOfReturnedcapitalCollectionsItems = $items.length;
          
          // add this to total number of returned items so that results only display if there are actually results to display
          totalItemsFound += numberOfReturnedcapitalCollectionsItems;
          
          if (numberOfReturnedcapitalCollectionsItems > 0) {            
            $('#capitalCollectionsResultsCounters').text(numberOfReturnedcapitalCollectionsItems);
            $('#searchResults').removeClass("invisible");
          }
 ...
 function buildPopupLayout(titlesArray, descriptionsArray) {
    ...
    for (var i = 0; i <= titlesArray.length - 1; i++) {
      $('#category_capitalCollections_popup').append("<h3>" + titlesArray[i] + "</h3>");       
      console.log(titlesArray[i]);
      
      $('#category_capitalCollections_popup').append("<p>" + descriptionsArray[i] + "</p>"); 
